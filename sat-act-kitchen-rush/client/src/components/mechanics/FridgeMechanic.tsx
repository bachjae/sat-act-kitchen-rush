...
export function FridgeMechanic({ onComplete, requiredIngredients }: FridgeMechanicProps) {
  ...
  // Default to 3 random required ingredients if not specified
  ...
  const handleSubmit = () => {
    const correct = required.every(r => selectedIngredients.includes(r)) &&
                    selectedIngredients.length === required.length;
    setGameOver(true);
    onComplete(correct);
  };

  const isCorrectSelection = () => {
    return required.every(r => selectedIngredients.includes(r)) &&
           selectedIngredients.length === required.length;
  };

  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-teal-50 rounded-xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden">
        {/* Header */}
        <div className="bg-teal-600 text-white px-6 py-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <span>🧊</span> Fridge Station
            </h2>
            <div className={`px-3 py-1 rounded-full font-mono text-lg ${
              timeLeft <= 5 ? 'bg-red-500 animate-pulse' : 'bg-teal-700'
            }`}>
              {timeLeft}s
            </div>
          </div>
        ...
        <div className="px-6 py-4 bg-teal-100 border-b border-teal-200">
          <p className="text-teal-800 font-medium">
            Gather the correct ingredients for your recipe!
          </p>
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="text-sm text-teal-600">Need:</span>
            {required.map(id => {
              const ing = AVAILABLE_INGREDIENTS.find(i => i.id === id);
              return (
                <span key={id} className={`px-2 py-1 rounded text-sm font-medium ${
                  selectedIngredients.includes(id) ? 'bg-green-500 text-white' : 'bg-white text-teal-800'
                }`}>
                  {ing?.emoji} {ing?.name}
                </span>
              );
              ...
              <button
                ...
                className={`
                  ...
                  ${selectedIngredients.includes(ingredient.id)
                    ? 'border-teal-500 bg-teal-100 scale-105 ring-2 ring-teal-300'
        ...
        <div className="px-6 pb-6">
          <button
            ...
            `}
          >
            {gameOver ? 'Time\'s Up!' : `Grab ${selectedIngredients.length} Items`}
          </button>
        </div>
      </div>
    </div>
